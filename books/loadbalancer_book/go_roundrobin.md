...
### ラウンドロビン方式
この方式はサーバの選択方式としては最もシンプルで基本的なアルゴリズムとなっています。
...
デメリットとして、1つのリクエストに時間がかかってしまう場合には、次々にリクエストが送られてきてしまうために特定のサーバに負荷が集中しやすくなる場合があり、待機時間が長くなってしまうことです。加えて、性能の低いサーバにはリクエストを処理しきれずにアクセスが溜まりやすく、効率が悪くなってしまいます。
...
具体的には、サーバ1、サーバ2、サーバ3が存在する場合にサーバ1にコネクションが2つ、サーバ2にコネクションが1つ、サーバ3にコネクションが3つあるとすると、次に送られてくるリクエストは接続数であるコネクションが最も少ないサーバ2に送られます。
...
実行手順としては、ターミナル上で`docker-compose up -d`というコマンドを打ち込み、コンテナを立ち上げます。
その後、ブラウザから`http://localhost:8080`を検索することで実行できます。
実行結果の確認方法として、Docker Desktop上からログを確認できます。
